"""Generate v6k held-out review CSV: 20 clips (subset of 100) with GT vs predictions."""
import json, csv, random

# Load predictions
preds = {}
with open("reports/b3_real_v6k_3k_heldout_predictions.jsonl") as f:
    for line in f:
        d = json.loads(line)
        preds[d["sample_id"]] = d

# Load held-out manifest
manifest = [json.loads(l) for l in open("/tmp/heldout_v6k_manifest.jsonl")]

# Select 20 diverse clips
rng = random.Random(42)
selected = rng.sample(manifest, 20)

out_path = "data/derived/real_mix_v6_1k/heldout_review_20.csv"
with open(out_path, "w", newline="", encoding="utf-8-sig") as f:
    ...
    for m in selected:
        ...
        gt_events = m["target_ledger"]["events"]
        gt_str = " | ".join(
...
print(f"Wrote 20 clips to {out_path}")
print("\nClips:")
for m in selected:
    sid = m["scene"]["scene_id"]
    pred = preds.get(sid, {})
