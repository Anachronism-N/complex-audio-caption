...
def main():
    ...
    n_mixtures = 1000
    out_dir = Path("/tmp/real_mix_v7")
    audio_dir = out_dir / "audio"
    audio_dir.mkdir(parents=True, exist_ok=True)
    rng = random.Random(20260816)
    # Enhanced prompt (docs/22 problem 26 fix)
    prompt = "Describe ALL audible events in this audio in detail, including sound type, timing, and characteristics."
    t0 = time.time()
    manifest = []
    ...
    for i in range(n_mixtures):
        ...
        for sc_idx, src_spec in enumerate(scene["sources"]):
            ...
            try:
                raw_path = f"/tmp/rv7_src_{sid}_{sc_idx}.wav"
        ...
        if (i+1) % 50 == 0:
            print(f"[rv7] {i+1}/{n_mixtures} ({time.time()-t0:.0f}s)", file=sys.stderr, flush=True)
